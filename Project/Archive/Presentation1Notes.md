My name is Corinne Fukayama and I am looking at being able to predict the general health for women. I'm a PH student majoring in global health epi and I've been studying cultural determinants of health disparities.
...
So my question combines the two concepts: can we predict an adult women’s general health based on existing structural disadvantages such as race and SES, and related, can we predict the change in health over time?
...
Since I'm looking at Race and SES as "indicators" for health outcomes but with the amount of information in the study, I can dig deeper into the other mediating variables of Race and SES, such as 
...
And then my allostatic load score, which is dependent on the 11 measures I've listed there, making this a regression problem.

###**Data Exploration part 2**###

Most of my time has been spent crawling through the codebooks (which are about 400 pgs long) and lit research trying to determine which variables and fields are representative of what I want to do. Now armed with a better idea of which fields I want to use, I've been able to start playing around with the data.
...
**The good issues**
So despite this, there are actually some really good things about these data sets. The main one being is that all of the data sets are relatively clean and easy to manipulate-- I haven't come across any user inputted errors, and everything is an integer or a float. The only issue I've come across is that missing values are coded as spaces.
